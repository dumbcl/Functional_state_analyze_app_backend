...
def get_trend_test_results(
    ...
    return TrendTestResult(
        ...
        estimation_result=get_fs_categories_by_date(db, user.id),
    )


def get_fs_categories_by_date(db: Session, user_id: int) -> List[DailyEstimationResult]:
    # Сбор всех дат
    test_dates = []
    test_dates += db.query(models.ShtangeTestResult.test_date).filter_by(user_id=user_id).distinct().all()
    test_dates += db.query(models.PersonalReportTestResult.test_date).filter_by(user_id=user_id).distinct().all()
    test_dates += db.query(models.RufieTestResult.test_date).filter_by(user_id=user_id).distinct().all()
    test_dates += db.query(models.StrupTestResult.test_date).filter_by(user_id=user_id).distinct().all()
    test_dates += db.query(models.GenchTestResult.test_date).filter_by(user_id=user_id).distinct().all()
    test_dates += db.query(models.ReactionsTestResult.test_date).filter_by(user_id=user_id).distinct().all()
    test_dates += db.query(models.TextAuditionResults.test_date).filter_by(user_id=user_id).distinct().all()
    ...
    for test_date in unique_test_dates:
        ...
        shtange_results = db.query(models.ShtangeTestResult).filter_by(user_id=user_id, test_date=test_date).all()
        shtange_result_indicator = shtange_results[0].reaction_indicator if shtange_results else None
        shtange_test_result_indicator_average = db.query(func.avg(models.ShtangeTestResult.reaction_indicator)).filter_by(user_id=user_id).scalar()

        personal_report_result = db.query(models.PersonalReportTestResult).filter_by(user_id=user_id, test_date=test_date).first()
        personal_report_indicator = personal_report_result.performance_measure if personal_report_result else None
        personal_report_all = db.query(models.PersonalReportTestResult).filter_by(user_id=user_id).all()
        ...
        pulse_measurements = db.query(models.PulseMeasurement).filter_by(user_id=user_id, created_at=test_date).all()
        pulseAverage = sum([pm.value for pm in pulse_measurements]) / len(pulse_measurements) if pulse_measurements else None
        pulse_alltime_measurements = db.query(models.PulseMeasurement).filter_by(user_id=user_id).all()
        ...
        rufie_results = db.query(models.RufieTestResult).filter_by(user_id=user_id, test_date=test_date).all()
        rufie_result_indicator = rufie_results[0].rufie_index if rufie_results else None
        rufie_test_result_indicator_average = db.query(func.avg(models.RufieTestResult.rufie_index)).filter_by(user_id=user_id).scalar()

        strup_results = db.query(models.StrupTestResult).filter_by(user_id=user_id, test_date=test_date).all()
        strup_result = strup_results[0].result if strup_results else None
        strup_test_result_average = db.query(func.avg(models.StrupTestResult.result)).filter_by(user_id=user_id).scalar()

        gench_results = db.query(models.GenchTestResult).filter_by(user_id=user_id, test_date=test_date).all()
        gench_result_indicator = gench_results[0].reaction_indicator if gench_results else None
        gench_test_result_indicator_average = db.query(func.avg(models.GenchTestResult.reaction_indicator)).filter_by(user_id=user_id).scalar()

        reactions_results = db.query(models.ReactionsTestResult).filter_by(user_id=user_id, test_date=test_date).all()
        ...
        reactions_visual_errors_average = db.query(func.avg(models.ReactionsTestResult.visual_errors)).filter_by(user_id=user_id).scalar()
        reactions_audio_errors_average = db.query(func.avg(models.ReactionsTestResult.audio_errors)).filter_by(user_id=user_id).scalar()

        text_audition_results = db.query(models.TextAuditionResults).filter_by(user_id=user_id, test_date=test_date).all()
        ...
        pauses_count_read_average = db.query(func.avg(models.TextAuditionResults.pauses_count_read)).filter_by(user_id=user_id).scalar()
        pauses_count_repeat_average = db.query(func.avg(models.TextAuditionResults.pauses_count_repeat)).filter_by(user_id=user_id).scalar()
        ...
        average_volume_read_average = db.query(func.avg(models.TextAuditionResults.average_volume_read)).filter_by(user_id=user_id).scalar()
        average_volume_repeat_average = db.query(func.avg(models.TextAuditionResults.average_volume_repeat)).filter_by(user_id=user_id).scalar()
...
@router.get("/trend-test-results-public", response_model=TrendTestResult)
def get_trend_test_results(
    user_id: int,
    db: Session = Depends(get_db),
    user: models.User = Depends(auth.get_current_user)
    ...
    # Shtange
    shtange_objs = db.query(models.ShtangeTestResult).filter_by(user_id=user_id).order_by(models.ShtangeTestResult.test_date).all()
    shtange_test_result = [
        evaluate_shtange(
            shtange_result=obj.result_estimation,
            shtange_result_indicator=obj.reaction_indicator,
            shtange_average=db.query(func.avg(models.ShtangeTestResult.reaction_indicator)).filter_by(user_id=user_id).scalar(),
            date=str(obj.test_date),
            test_id=obj.testing_id,
        )
        for obj in shtange_objs
    ...
    # PersonalReport
    personal_report_objs = db.query(models.PersonalReportTestResult).filter_by(user_id=user_id).order_by(models.PersonalReportTestResult.test_date).all()
    all_perf = [o.performance_measure for o in personal_report_objs]
    avg_perf = sum(all_perf) / len(all_perf) if all_perf else None
    personal_report = [
        evaluate_personal_report(
    ...
    # PulseMeasurement
    pulse_objs = db.query(models.PulseMeasurement).filter_by(user_id=user_id).order_by(models.PulseMeasurement.created_at).all()
    all_pulse_values = [o.value for o in pulse_objs]
    pulse_alltime_avg = sum(all_pulse_values) / len(all_pulse_values) if all_pulse_values else None
    pulse_measurement = [
        evaluate_pulse(
    ...
    # Rufie
    rufie_objs = db.query(models.RufieTestResult).filter_by(user_id=user_id).order_by(models.RufieTestResult.test_date).all()
    all_rufie = [o.rufie_index for o in rufie_objs if o.rufie_index is not None]
    avg_rufie = sum(all_rufie) / len(all_rufie) if all_rufie else None
    rufie_test_result = [
        evaluate_rufie(
    ...
    # Strup
    strup_objs = db.query(models.StrupTestResult).filter_by(user_id=user_id).order_by(models.StrupTestResult.test_date).all()
    all_strup = [o.result for o in strup_objs if o.result is not None]
    avg_strup = sum(all_strup) / len(all_strup) if all_strup else None
    strup_test_result = [
        evaluate_strup(
    ...
    # Gench
    gench_objs = db.query(models.GenchTestResult).filter_by(user_id=user_id).order_by(models.GenchTestResult.test_date).all()
    all_gench = [o.reaction_indicator for o in gench_objs if o.reaction_indicator is not None]
    avg_gench = sum(all_gench) / len(all_gench) if all_gench else None
    gench_test_result = [
        evaluate_gench(
    ...
    # Reactions
    reactions_objs = db.query(models.ReactionsTestResult).filter_by(user_id=user_id).order_by(models.ReactionsTestResult.test_date).all()
    all_visual = [o.visual_errors for o in reactions_objs if o.visual_errors is not None]
    all_audio = [o.audio_errors for o in reactions_objs if o.audio_errors is not None]
    avg_visual = sum(all_visual) / len(all_visual) if all_visual else None
    avg_audio = sum(all_audio) / len(all_audio) if all_audio else None
    ...
    # TextAudition
    text_objs = db.query(models.TextAuditionResults).filter_by(user_id=user_id).order_by(models.TextAuditionResults.test_date).all()
    all_pauses_read = [o.pauses_count_read for o in text_objs if o.pauses_count_read is not None]
    all_pauses_repeat = [o.pauses_count_repeat for o in text_objs if o.pauses_count_repeat is not None]
    all_vol_read = [o.average_volume_read for o in text_objs if o.average_volume_read is not None]
    all_vol_repeat = [o.average_volume_repeat for o in text_objs if o.average_volume_repeat is not None]
    ...
    # EscalDaily
    escal_objs = db.query(models.EscalDailyResults).filter_by(user_id=user_id).order_by(models.EscalDailyResults.test_date).all()
    escal_daily_test_result = [
        evaluate_escal_daily(
            performance=obj.performance,
            fatigue=obj.fatigue,
